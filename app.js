...
(() => {
  ...
  function launchReady(){
    return `<section class="section" id="launch"><div class="container section-head"><div><div class="eyebrow">Public launch readiness</div><h2>Live now in controlled beta mode.</h2></div><p>Do not open paid public checkout until production gates pass. Add real credentials only as Render environment variables.</p></div><div class="container grid cols-2"><article class="panel"><h3>Launch gates</h3><ul class="feature-list compact">${launchGates.map(x => `<li>${escapeHtml(x)}</li>`).join('')}</ul></article><article class="panel warning-card"><h3>No-data-loss deployment rule</h3><p>Deployments must protect ${protectedRecords.length} record types including ${protectedRecords.slice(0,10).join(', ')} and more.</p><p class="notice">No destructive migrations. No hard-coded live secrets. PostgreSQL and object storage are required before real users save important records.</p></article></div></section>`;
  }
  function trust(){
    return `<section class="section alt" id="trust"><div class="container section-head"><div><div class="eyebrow">Trust center</div><h2>Clear boundaries protect students, teachers, employers, and the brand.</h2></div><p>EnglishLegalese is education, language training, and professional communication practice only.</p></div><div class="container grid cols-3">${card('Not a law firm','The platform does not provide legal advice or decide legal rights, obligations, deadlines, filings, or strategy.')}${card('Not certified translation','Native-language explanations help learning. They are not certified translation, court interpretation, or final legal meaning.')}${card('Private by default','Employer reports are privacy-safe. Private learner drafts, uploads, raw AI practice, and teacher-private notes are not shared by default.')}</div></section>`;
  }
